# Gryst TTRPG

> Uhhh lets make a ttrpg I guess.

## What is Gryst?

Gryst is a TTRPG built around dynamic initiative and collaborative action. Instead of rolling for turn order once and cycling through, players **race** to earn their turns through a **Momentum** system — and when it's time to act, everyone can jump in through **Add-Ons** to build a stack of gambits that resolve as one cinematic moment.

## Quick Links

- **[Vision Statement](core-design/Vision-Statement.md)** — What we're building and why
- **[Core Mechanic](rules/Core-Mechanic.md)** — How dice work
- **[Momentum & Turns](rules/Momentum-and-Turns.md)** — The heart of the system
- **[Gambits](rules/Gambits.md)** — The atomic unit of action resolution

## Core Concepts

### Momentum
Every character has a Momentum tracker (d20 counter). Each "tick," everyone adds their Momentum Gain simultaneously. First to hit 20 becomes the **Pivotal Character** and kicks off a resolution phase.

### The Stack
When the Pivotal Character acts, allies can **Add-On** — spending their own momentum to slot actions before or after in a queue. The whole stack resolves as one narrative beat, not turn-by-turn ping-pong.

### Gambits
Every action is a **Gambit** with four possible outcomes:
- **Triumphus** — Critical success
- **Successus** — Success  
- **Clades** — Failure
- **Calamitas** — Critical failure

---

*This documentation is a living design document. Expect things to change as we playtest.*

Most of this site is AI generated placeholder template, only some basic worldbuilding and core rules are designed right now! 